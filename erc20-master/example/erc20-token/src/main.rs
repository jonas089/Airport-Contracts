...
use casper_types::{account::AccountHash, ApiError, CLValue, U256};
...
pub extern "C" fn mint() {
    let installer_uref = match runtime::get_key("INSTALLER") {
        Some(uref) => uref,
    ...
    let installer: AccountHash = storage::read_or_revert(installer_uref);
    if installer != runtime::get_caller() {
        runtime::revert(ApiError::PermissionDenied);
    }
    let owner = Address::from(runtime::get_caller());
...
fn call() {
    ...
    let _token = ERC20::install(name, symbol, decimals, total_supply, installer).unwrap_or_revert();
}
