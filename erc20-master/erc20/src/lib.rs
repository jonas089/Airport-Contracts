...
use casper_types::{
    account::AccountHash, runtime_args, CLType, CLValue, ContractHash, ContractPackageHash,
    ContractVersion, EntryPoint, EntryPointAccess, EntryPointType, KeyTag, Parameter, RuntimeArgs,
    Tagged,
};
...
impl ERC20 {
    ...
    pub fn install_custom(
        ...
        let installer_key = {
            let installer_uref = storage::new_uref(installer).into_read();
            Key::from(installer_uref)
        };
        ...
        named_keys.insert("INSTALLER".to_string(), installer_key);
        ...
        // mint entry point does not need to be part of ERC20 impl.
