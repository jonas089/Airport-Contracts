...
use casper_types::{
    ...
    EntryPoints, Key, KeyTag, Parameter, RuntimeArgs, Tagged, URef, U256,
};
...
pub extern "C" fn mint() {
    ...
    // if mint_pre_condition_hash.exists() { let value = runtime::call_contract(pre_condition_hash,
    // runtime_args).unwrap() } if mint_post_condition_hash.exists() { let val =
    // runtime::call_contract(post_condition_hash, runtime_args).unwrap() }
...
pub extern "C" fn call() {
    ...
    let collection_name: String = String::from("AirportCEP78");

    // TODO: figure out examples of collection_symbol
    // The symbol for the NFT collection.
    // This value cannot be changed after installation.
    let collection_symbol: String = String::from("air78");

    // This represents the total number of NFTs that will
    // be minted by a specific instance of a contract.
    // This value cannot be changed after installation.

    // for prototyping this is set to a very high value.
    // in production app there would be no token limit.
    let total_token_supply: u64 = 1000_000_000_000_000_000;
    ...
    // 0: minter 1: assigned 2: transferable
    ...
    // metadata schema used in Tickets:
    /*

    '{\"properties\":{\"ticket_data\":{\"name\":\"ticket_data\",\"description\":\"data_of_the_ticket_as_json_string\",\"required\":true},\"user_data\":{\"name\":\"user_data\",\"description\":\"data_of_the_user_as_json_string\",\"required\":true},}}'

        */
    // when calling mint() pass 2 arguments, "ticket_data"
    // and "user_data". Both should be json-strings.
    // To get the metadata, query the "metadata_custom_validated" dictionary,
    // using the NFT hash identifier as a key.
